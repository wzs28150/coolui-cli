...
function init(args) {
    ...
    console.log(
        CooluiCli + (!isCn ? ' will execute init command... ' : ' 即将执行 init 命令...')
    )
    ...
    function createApp() {
        console.log()
        console.log(
            chalk.bold.cyan('Coolui-Cli') + (!isCn ? ' will creating a new Coolui app in ' : ' 即将创建一个新的应用在 ') + dest
        )
    ...
    function done() {
        ...
        console.log()
        console.log(`${chalk.bold.cyan('Coolui')} https://github.com/wzs28150/coolui-weapp`)
    }
}
